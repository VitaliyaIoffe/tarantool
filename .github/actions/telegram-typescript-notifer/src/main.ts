...
async function run(): Promise<void> {
  try {
    ...
    const chatId: string | number = await checkBranchName(branchName);
...
async function sendMessage(
  ...
  const BRANCH_NAME = await setEnvironmentVariables();
...
async function checkBranchName(branchName: string): Promise<string> {
  const chatId = core.getInput("to");
  const actor = github.context.actor;

  const branchArr: Array<string> = [
    "refs/heads/master",
    "refs/heads/1.10",
    "refs/heads/2",
    "refs/tags",
  ...
  if (!branchArr.includes(branchName)) {
    return chatId + "_" + actor;
  }
  return chatId;
}

async function setEnvironmentVariables(): Promise<string | undefined> {
  const branchName: string = github.context.ref;
